  ...
  --*/
{
	...
	if (wNumWord >= PAL_ADDITIONAL_WORD_SECOND)
	{
		return PAL_New_GetNewWord(wNumWord - PAL_ADDITIONAL_WORD_SECOND);
	}
	else if (wNumWord >= PAL_ADDITIONAL_WORD_FIRST)
	{
  ...
  --*/
{
	...
	if (wNumMsg >= PAL_ADDITIONAL_MSG_SECOND)
	{
		return PAL_NEW_GetNewMsg(gpGlobals->lpObjectNewMsg, wNumMsg - PAL_ADDITIONAL_MSG_SECOND);
	}
...
)
{
	...
	char             buf[512];
	char                      *p;
	LPOBJECTNEWMSG               head = NULL, lp = NULL, node = NULL;

	memset(&g_NewTextLib, 0, sizeof(g_NewTextLib));

	fpMsg = UTIL_OpenFile("new_m.msg");
	fpWord = UTIL_OpenFile("new_word.dat");

	if (fpMsg == NULL || fpWord == NULL)
	{
		return -1;
	}
	...
	// Each word has 10 bytes
	//
	g_NewTextLib.nWords = (i - 2) / WORD_LENGTH;

	//
	// Read the words
	//
	g_NewTextLib.lpWordBuf = (LPBYTE)malloc(i);
	if (g_NewTextLib.lpWordBuf == NULL)
	{
		fclose(fpWord);
		fclose(fpMsg);
		return -1;
	...
	fseek(fpWord, 0, SEEK_SET);
	fread(g_NewTextLib.lpWordBuf, i, 1, fpWord);
	...
	//
	while (fgets(buf, 512, fpMsg) != NULL)
	{
		// 查找字符串s中首次出现字符c的位置。
		p = strchr(buf, '\r');
		if (p == NULL)
		...
		*p = '\0';
		...
		node = UTIL_calloc(1, sizeof(OBJECTNEWMSG));
		node->lpMsg = UTIL_calloc(1, strlen(buf) + 1);
		strcpy(node->lpMsg, buf);
	...
	g_NewTextLib.bCurrentFontColor = FONT_COLOR_DEFAULT;
	g_NewTextLib.bIcon = 0;
	g_NewTextLib.posIcon = 0;
	g_NewTextLib.nCurrentDialogLine = 0;
	g_NewTextLib.iDelayTime = 3;
	g_NewTextLib.posDialogTitle = PAL_XY(12, 8);
	g_NewTextLib.posDialogText = PAL_XY(44, 26);
	g_NewTextLib.bDialogPosition = kDialogUpper;
	g_NewTextLib.fUserSkip = FALSE;

	gpGlobals->lpObjectNewMsg = head;
...
VOID
PAL_New_FreeNewText(
LPOBJECTNEWMSG   lpObjectNewMsg
)
{
	if (g_NewTextLib.lpWordBuf != NULL)
	{
		free(g_NewTextLib.lpWordBuf);
		g_NewTextLib.lpWordBuf = NULL;
	}

	LPOBJECTNEWMSG    p;

	while (lpObjectNewMsg != NULL)
	{
		p = lpObjectNewMsg->next;
		free(lpObjectNewMsg->lpMsg);
		free(lpObjectNewMsg);
		lpObjectNewMsg = p;
	}
}
...

{
	...
	if (wNumWord >= g_NewTextLib.nWords)
	{
		return NULL;
	}

	memcpy(buf, &g_NewTextLib.lpWordBuf[wNumWord * WORD_LENGTH], WORD_LENGTH);
	buf[WORD_LENGTH] = '\0';
...
//C语言中,形参是实参的副本,包括指针，指针的内容也许会在调用函数后变化，但指针本身不会
LPCSTR
PAL_NEW_GetNewMsg(
LPOBJECTNEWMSG   lpObjectNewMsg,
WORD       wNumMsg
)
{
	...
	while (lpObjectNewMsg != NULL)
	{
		if (i == wNumMsg)
		{
			return lpObjectNewMsg->lpMsg;
		}
		else
		{
			lpObjectNewMsg = lpObjectNewMsg->next;
			i++;
		}
