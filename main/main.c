...
/*++
  ...
  Show the trademark screen.开场动画1
  ...
  --*/
{
	...
	//SDL_Surface   *surBitmapCrane, *surBitmapTitle;
#endif
...
#ifdef BIG_RESOLUTION
// 	surBitmapCrane = SDL_CreateRGBSurface(gpScreen->flags, 320, 200, 8,
// 		gpScreen->format->Rmask, gpScreen->format->Gmask, gpScreen->format->Bmask,
// 		gpScreen->format->Amask);
// 	surBitmapTitle = SDL_CreateRGBSurface(gpScreen->flags, 320, 200, 8,
// 		gpScreen->format->Rmask, gpScreen->format->Gmask, gpScreen->format->Bmask,
// 		gpScreen->format->Amask);
#endif
...
#ifdef BIG_RESOLUTION
	//SDL_SetSurfacePalette(surBitmapCrane, gpScreen->format->palette);
	//SDL_SetSurfacePalette(surBitmapTitle, gpScreen->format->palette);
#endif
#else
...
// 		dstrect.x = 0;
// 		dstrect.y = 0;
// 		dstrect.h = g_iSurfaceHeight;
// 		dstrect.w = g_iSurfaceWidth;
// 		SDL_SoftStretch(lpBitmapUp, NULL, gpScreen, &dstrect);
...
#ifdef BIG_RESOLUTION
// 		dstrect.x = 0;
// 		dstrect.y = (200 - iImgPos) * g_iSurfaceHeight / 200;
// 		dstrect.h = g_iSurfaceHeight;
// 		dstrect.w = g_iSurfaceWidth;
// 		SDL_SoftStretch(lpBitmapDown, NULL, gpScreen, &dstrect);
  ...
  --*/
{
...
#ifdef BIG_RESOLUTION
		wScreenWidth = 960;
		wScreenHeight = fFullScreen ? 720 : 600;
#else
		wScreenWidth = 640;
	...
	//
#ifdef PAL_HAS_AVI
	if (access(va("%s/avi/1.AVI", PAL_PREFIX), 0) == 0)
	{
		PAL_PlayAVI(va("%s/avi/1.AVI", PAL_PREFIX), TRUE);
	}
	else
	{
		PAL_TrademarkScreen();
	}
	if (access(va("%s/avi/2.AVI", PAL_PREFIX), 0) == 0)
	{
		PAL_PlayAVI(va("%s/avi/2.AVI", PAL_PREFIX), TRUE);
	}
	else
	{
		PAL_SplashScreen();
